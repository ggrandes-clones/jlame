...
final class Jencoder {
	...
	*/
	...
	private static final void lame_encode_frame_init(final Jlame_internal_flags gfc, final float inbuf[/* 2 */][] ) {
		final JSessionConfig cfg = gfc.cfg;
	...
	*/
	 ...
	 * @return
	 */
	static final int lame_encode_mp3_frame(final Jlame_internal_flags gfc, final float[] inbuf_l, final float[] inbuf_r, final byte[] mp3buf, final int mp3buf_offset, final int mp3buf_size)
	{
		final JSessionConfig cfg = gfc.cfg;
		...
		****************************************/
		{
			...
			for( int gr = 0; gr < mode_gr; gr++ ) {
				...
				final int ret = Jpsymodel.L3psycho_anal_vbr( gfc, inbuf, bufp, gr,
		...
		if( cfg.analysis && gfc.pinfo != null ) {
			...
			for( int gr = 0; gr < mode_gr; gr++ ) {
				...
				do {
					...
					gfc.pinfo.blocktype[gr][ch] = tt_gr[ch].block_type;
					gfc.pinfo.pe[gr][ch] = pe_use[gr][ch];
					// memcpy(gfc.pinfo.xr[gr][ch], &gfc.l3_side.tt[gr][ch].xr[0], sizeof(float) * 576);// FIXME double <-> float size mismatch
					System.arraycopy( tt_gr[ch].xr, 0, gfc.pinfo.xr[gr][ch], 0, 576 );
					/* in psymodel, LR and MS data was stored in pinfo.
					   switch to MS data: */
					if( gfc.ov_enc.mode_ext == MPG_MD_MS_LR ) {
						final double[] ers_gr = gfc.pinfo.ers[gr];// java
						ers_gr[ch] = ers_gr[ch + 2];
		...
		****************************************/
		...
		if( cfg.analysis && gfc.pinfo != null ) {
			final int framesize = 576 * mode_gr;
			final double[][] pcmdata = gfc.pinfo.pcmdata;// java

			int ch = 0;
			do {
				final double[] pcmdata_ch = pcmdata[ch];// java
				final float[] inbuf_ch = inbuf[ch];// java
				int j = 0;
				for( ; j < FFTOFFSET; j++ ) {
					pcmdata_ch[j] = pcmdata_ch[j + framesize];
				}
				for( ; j < 1600; j++ ) {
					pcmdata_ch[j] = inbuf_ch[j - FFTOFFSET];
				}
			} while( ++ch < channels_out );
			gfc.sv_qnt.masking_lower = 1.0f;

			Jquantize_pvt.set_frame_pinfo( gfc, masking );
		}
