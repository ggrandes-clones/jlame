...
final class JABR_iteration_loop extends Jquantize/* implements Jiteration_loop */{
	 ...
	 ********************************************************************/
	private static final long calc_target_bits(final Jlame_internal_flags gfc,
			final float pe[][]/*[2][2]*/,
			...
			final int targ_bits[][]/*[2][2]*//*, int[] analog_silence_bits, int[] max_frame_bits*/)
	{
		...
		if( gfc.ov_enc.mode_ext == Jencoder.MPG_MD_MS_LR ) {
			...
			for( int gr = 0; gr < mode_gr; gr++ ) {
				Jquantize_pvt.reduce_side( targ_bits[gr], ms_ener_ratio[gr], mean_bits, Jutil.MAX_BITS_PER_GRANULE );
			}
		}
	 ...
	 *
	 ********************************************************************/
	//private static final void ABR_iteration_loop(final Jlame_internal_flags gfc, final float pe[][]/*[2][2]*/,
