...
public final class Jlame_global_flags {// struct lame_global_struct
	...
	boolean findReplayGain;
	/** decode on the fly? default=0 */
	boolean decode_on_the_fly;
	...
	/* struct {
	void    (*msgf) (const char *format, final va_list ap);
	void    (*debugf) (const char *format, final va_list ap);
	void    (*errorf) (const char *format, final va_list ap);
	} report; */
	...
	/** input samplerate */
	public final boolean lame_set_in_samplerate(final int in_samplerate) {
		if( is_lame_global_flags_valid() ) {
			if( in_samplerate < 1 ) {
				return true;
			}
	...
	public final int lame_get_in_samplerate() {
		if( is_lame_global_flags_valid() ) {
	...
	public final int lame_set_scale_left(final float left_scale) {
		if( is_lame_global_flags_valid() ) {
			/* default = 1 */
			this.scale_left = left_scale;
			return 0;
		}
		return -1;
	}

	public final float lame_get_scale_left() {
		if( is_lame_global_flags_valid() ) {
	...
	/** scale the channel 1 (right) input by this amount before
	   encoding (not used for decoding) */
	public final int lame_set_scale_right(final float right_scale) {
		if( is_lame_global_flags_valid() ) {
			/* default = 1 */
			this.scale_right = right_scale;
			return 0;
		}
		return -1;
	}

	public final float lame_get_scale_right() {
		if( is_lame_global_flags_valid() ) {
	...
	public final int lame_set_out_samplerate(final int out_samplerate) {
		if( is_lame_global_flags_valid() ) {
			/*
			 ...
			 */
			if( out_samplerate != 0 ) {
				if( Jutil.SmpFrqIndex( out_samplerate ) < 0 ) {
					return -1;
				}
			}
	...
	public final int lame_set_analysis(final boolean is_analysis) {
		if( is_lame_global_flags_valid() ) {
			/* default = 0 */
			this.analysis = is_analysis;
			return 0;
		}
		return -1;
	}

	public final boolean lame_get_analysis() {
		if( is_lame_global_flags_valid() ) {
	...
	public final int lame_set_findReplayGain(final boolean is_findReplayGain) {
		if( is_lame_global_flags_valid() ) {
			/* default = 0 (disabled) */
			this.findReplayGain = is_findReplayGain;
			return 0;
		}
		return -1;
	}

	public final boolean lame_get_findReplayGain() {
		if( is_lame_global_flags_valid() ) {
	...
	public final int lame_set_decode_on_the_fly(final boolean is_decode_on_the_fly) {
		if( is_lame_global_flags_valid() ) {
			/* default = 0 (disabled) */
			this.decode_on_the_fly = is_decode_on_the_fly;
	...
	public final boolean lame_get_decode_on_the_fly() {
		if( is_lame_global_flags_valid() ) {
	...
	/* message handlers */
	public final int lame_set_errorf(final PrintStream func) {
		if( is_lame_global_flags_valid() ) {
	...
	public final int lame_set_debugf(final PrintStream func) {
		if( is_lame_global_flags_valid() ) {
	...
	public final int lame_set_msgf(final PrintStream func) {
		if( is_lame_global_flags_valid() ) {
	...
	public final int lame_set_allow_diff_short(final boolean allow_diff_short) {
		if( is_lame_global_flags_valid() ) {
			this.short_blocks = allow_diff_short ? Jlame_global_flags.short_block_allowed : Jlame_global_flags.short_block_coupled;
			return 0;
		}
		return -1;
	}
	...
	public final int lame_set_no_short_blocks(final boolean no_short_blocks) {
		if( is_lame_global_flags_valid() ) {
			this.short_blocks = no_short_blocks ? Jlame_global_flags.short_block_dispensed : Jlame_global_flags.short_block_allowed;
			return 0;
		}
		return -1;
	}
	...
	*/
	...
	public final int lame_get_RadioGain() {
		if( is_lame_global_flags_valid() ) {
			final Jlame_internal_flags gfc = this.internal_flags;
			if( gfc.is_lame_internal_flags_valid() ) {
	...
	public final int lame_get_AudiophileGain() {
		if( is_lame_global_flags_valid() ) {
			final Jlame_internal_flags gfc = this.internal_flags;
			if( gfc.is_lame_internal_flags_valid() ) {
				return 0;
	...
	/** the peak sample */
	public final float lame_get_PeakSample() {
		if( is_lame_global_flags_valid() ) {
			final Jlame_internal_flags gfc = this.internal_flags;
			if( gfc.is_lame_internal_flags_valid() ) {
	...
	public final int lame_get_noclipGainChange() {
		if( is_lame_global_flags_valid() ) {
			final Jlame_internal_flags gfc = this.internal_flags;
			if( gfc.is_lame_internal_flags_valid() ) {
	...
	public final float lame_get_noclipScale() {
		if( is_lame_global_flags_valid() ) {
			final Jlame_internal_flags gfc = this.internal_flags;
			if( gfc.is_lame_internal_flags_valid() ) {
	...
	public final int lame_get_totalframes() {
		if( is_lame_global_flags_valid() ) {
			...
			if( gfc.is_lame_internal_flags_valid() ) {
				...
				if( pcm_samples_to_encode == ((0L - 1L) & 0xffffffff) ) {// FIXME (0ul-1ul) must be MAX_U_32_NUM
					return 0; /* unknown */
				}
	...
	*/
	 ...
	 */
	final void lame_set_tune(final float val) {
		if( is_lame_global_flags_valid() ) {
...
*/
	private static final int calc_maximum_input_samples_for_buffer_size(final Jlame_internal_flags gfc, final int buffer_size)
	{
		final JSessionConfig cfg = gfc.cfg;
		...
		} else if( cfg.vbr == Jlame.vbr_off ) {
			kbps = cfg.avg_bitrate;
		}
		{
			final int pad = 1;
			final int bpf = ((cfg.version + 1) * 72000 * kbps / cfg.samplerate_out + pad);
			frames_per_buffer = buffer_size / bpf;
		}
		{
			final double ratio = (double) cfg.samplerate_in / cfg.samplerate_out;
			input_samples_per_buffer = (int)(pcm_samples_per_frame * frames_per_buffer * ratio);
		}
		return input_samples_per_buffer;
	}

	/**
	 * @return the limit of PCM samples, which one can pass in an encode call
	 * under the constrain of a provided buffer of size buffer_size
	 */
	public final int lame_get_maximum_number_of_samples(final int buffer_size)
	{
		if( is_lame_global_flags_valid() ) {
			final Jlame_internal_flags gfc = this.internal_flags;
			if( gfc.is_lame_internal_flags_valid() ) {
		...
		return -1;// FIXME lame_errorcodes_t never uses, LAME_GENERICERROR is replaced by -1;
	}
}
