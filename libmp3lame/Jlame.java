...
public final class Jlame {
	...
	private static final void lame_init_params_ppflt(final Jlame_internal_flags gfc ) {
		...
		if( cfg.highpass2 > 0 ) {
			if( cfg.highpass2 < .9f * (.75f / 31.0f) ) {
				cfg.highpass1 = 0;
				cfg.highpass2 = 0;
				Jutil.lame_msgf( gfc, "Warning: highpass filter disabled. highpass frequency too small\n"  );
			}
		}
	...
	private static final int optimum_samplefreq(final int lowpassfreq, final int input_samplefreq ) {
		/*
		 * Rules:
	...
	private static final class Jq_map {
		...
		//
		private Jq_map(final int sr, final float a, final float b, final float c, final float d/*, final int lfr*/) {
			this.sr_a = sr;
			this.qa = a;
			this.qb = b;
			this.ta = c;
	...
	public static final int lame_init_params(final Jlame_global_flags gfp) {
		...
		if( cfg.vbr == vbr_off && gfp.compression_ratio > 0 ) {
			...
			if( gfp.samplerate_out == 0 ) {
				gfp.samplerate_out = Jutil.map2MP3Frequency((int) (0.97f * gfp.samplerate_in) );
			} /* round up with a margin of 3% */

			/* choose a bitrate for the output samplerate which achieves
			 * specified compression ratio
			 */
			gfp.brate = (int)(gfp.samplerate_out * (cfg.channels_out << 4) / (1.e3f * gfp.compression_ratio));

			/* we need the version for the bitrate table look up */
			cfg.samplerate_index = Jutil.SmpFrqIndex( gfp.samplerate_out );
			cfg.version = Jutil.getVersion( gfp.samplerate_out );
			// assert( cfg->samplerate_index >= 0 );

			if( ! cfg.free_format ) {
				gfp.brate = Jutil.FindNearestBitrate( gfp.brate, cfg.version, gfp.samplerate_out );
			}
		}
		if( gfp.samplerate_out != 0 ) {
		...
		gfp.lowpassfreq = Math.min( gfp.samplerate_out / 2, gfp.lowpassfreq );

		if( cfg.vbr == vbr_off ) {
			gfp.compression_ratio = gfp.samplerate_out * (cfg.channels_out << 4) / (1.e3f * gfp.brate );
		}
		if( cfg.vbr == vbr_abr ) {
			gfp.compression_ratio =
					gfp.samplerate_out * (cfg.channels_out << 4) / (1.e3f * gfp.VBR_mean_bitrate_kbps );
		}
		...
		case vbr_abr:
			gfp.compression_ratio =
				cfg.samplerate_out * (cfg.channels_out << 4) / (1.e3f * gfp.VBR_mean_bitrate_kbps );
			break;
		default:
			gfp.compression_ratio = cfg.samplerate_out * (cfg.channels_out << 4) / (1.e3f * gfp.brate );
			break;
		}
		...
		cfg.lowpass1 = 0;
		cfg.lowpass2 = 0;
		if( cfg.lowpassfreq > 0 && cfg.lowpassfreq < (cfg.samplerate_out >> 1)  ) {
		...
		*******************************************************/
		cfg.samplerate_index = Jutil.SmpFrqIndex( cfg.samplerate_out );
		cfg.version = Jutil.getVersion( cfg.samplerate_out );
		// assert( cfg.samplerate_index >= 0 );
		...
		if( cfg.vbr == vbr_off ) {
			if( cfg.free_format ) {
				...
				gfp.brate = Jutil.FindNearestBitrate( gfp.brate, cfg.version, cfg.samplerate_out );
				gfc.ov_enc.bitrate_index = Jutil.BitrateIndex( gfp.brate, cfg.version, cfg.samplerate_out );
		...
		final int j = cfg.samplerate_index + (3 * cfg.version) + (cfg.samplerate_out < 16000 ? 6 : 0);
		...
		if( cfg.vbr != vbr_off ) { /* choose a min/max bitrate for VBR */
			...
			if( gfp.VBR_min_bitrate_kbps != 0 ) {
				gfp.VBR_min_bitrate_kbps =
						Jutil.FindNearestBitrate( gfp.VBR_min_bitrate_kbps, cfg.version, cfg.samplerate_out );
				cfg.vbr_min_bitrate_index =
						Jutil.BitrateIndex( gfp.VBR_min_bitrate_kbps, cfg.version, cfg.samplerate_out );
			...
			if( gfp.VBR_max_bitrate_kbps != 0 ) {
				gfp.VBR_max_bitrate_kbps =
						Jutil.FindNearestBitrate( gfp.VBR_max_bitrate_kbps, cfg.version, cfg.samplerate_out );
				cfg.vbr_max_bitrate_index =
						Jutil.BitrateIndex( gfp.VBR_max_bitrate_kbps, cfg.version, cfg.samplerate_out );
			...
			gfp.VBR_mean_bitrate_kbps =
					Math.min( Jtables.bitrate_table[cfg.version][cfg.vbr_max_bitrate_index], gfp.VBR_mean_bitrate_kbps );
			gfp.VBR_mean_bitrate_kbps =
					Math.max( Jtables.bitrate_table[cfg.version][cfg.vbr_min_bitrate_index], gfp.VBR_mean_bitrate_kbps );
		}
		 ...
		 */
		{
			...
			k[0][0] *= gfp.scale_left;
			k[0][1] *= gfp.scale_left;
			/* user selected scaling of the channel 1 (right) samples */
			k[1][0] *= gfp.scale_right;
			k[1][1] *= gfp.scale_right;
		...
		if( cfg.vbr == vbr_off ) {
			gfc.sv_enc.slot_lag = gfc.sv_enc.frac_SpF = (int)(((cfg.version + 1) * 72000L * cfg.avg_bitrate) % cfg.samplerate_out);
		}
		...
		if( cfg.decode_on_the_fly ) {
			cfg.findPeakSample = true;
		}

		if( cfg.findReplayGain ) {
			if( gfc.sv_rpg.rgdata.InitGainAnalysis( gfp.samplerate_out ) == Jreplaygain.INIT_GAIN_ANALYSIS_ERROR ) {
				/* Actually this never happens, our samplerates are the ones RG accepts!
				 * But just in case, turn RG off
				 */
				// assert(0);
				cfg.findReplayGain = false;
			}
		}
...
// #ifdef DECODE_ON_THE_FLY
		if( cfg.decode_on_the_fly && ! gfp.decode_only ) {
			if( gfc.hip != null ) {
				gfc.hip.hip_decode_exit();
			}
			gfc.hip = Jhip.hip_decode_init();
			/* report functions */
			gfc.hip.hip_set_errorf( gfp.report.errorf  );
			gfc.hip.hip_set_debugf( gfp.report.debugf  );
			gfc.hip.hip_set_msgf( gfp.report.msgf  );
		}
// #endif
	...
	public static final void lame_print_config(final Jlame_global_flags gfp) {
		final Jlame_internal_flags gfc = gfp.internal_flags;
		final JSessionConfig cfg = gfc.cfg;
		final double out_samplerate = cfg.samplerate_out;
		final double in_samplerate = cfg.samplerate_in;

		Jutil.lame_msgf( gfc, "LAME %s %s (%s)\n", Jlame_version.get_lame_version(), Jlame_version.get_lame_os_bitness(), Jlame_version.get_lame_url() );

		if( cfg.channels_in == 2 && cfg.channels_out == 1 /* mono */  ) {
			Jutil.lame_msgf(gfc, "Autoconverting from stereo to mono. Setting encoding to mono mode.\n" );
		}

		if( Jutil.isResamplingNecessary( cfg ) ) {
			Jutil.lame_msgf( gfc, "Resampling:  input %g kHz  output %g kHz\n",
					1.e-3f * in_samplerate, 1.e-3f * out_samplerate );
		}

		if( cfg.highpass2 > 0.) {
			Jutil.lame_msgf( gfc,
					"Using polyphase highpass filter, transition band: %5.0f Hz - %5.0f Hz\n",
					0.5f * cfg.highpass1 * out_samplerate, 0.5f * cfg.highpass2 * out_samplerate );
		}
		if( 0. < cfg.lowpass1 || 0. < cfg.lowpass2 ) {
			Jutil.lame_msgf( gfc,
					"Using polyphase lowpass filter, transition band: %5.0f Hz - %5.0f Hz\n",
					0.5f * cfg.lowpass1 * out_samplerate, 0.5f * cfg.lowpass2 * out_samplerate );
		} else {
			Jutil.lame_msgf( gfc, "polyphase lowpass filter disabled\n" );
		}

		if( cfg.free_format ) {
			Jutil.lame_msgf( gfc, "Warning: many decoders cannot handle free format bitstreams\n" );
	 ...
	 */
	@SuppressWarnings("boxing")
	public static final void lame_print_internals(final Jlame_global_flags gfp) {
		final Jlame_internal_flags gfc = gfp.internal_flags;
		final JSessionConfig cfg = gfc.cfg;
		...
		Jutil.lame_msgf( gfc, "\tscaling: %g\n", gfp.scale );
		Jutil.lame_msgf( gfc, "\tch0 (left) scaling: %g\n", gfp.scale_left );
		Jutil.lame_msgf( gfc, "\tch1 (right) scaling: %g\n", gfp.scale_right );
		...
		Jutil.lame_msgf( gfc, "\thuffman search: %s\n", pc );
		Jutil.lame_msgf( gfc, "\texperimental Y=%d\n", gfp.experimentalY );
		Jutil.lame_msgf( gfc, "\t...\n" );
		...
		Jutil.lame_msgf( gfc, "\tMPEG-%s Layer 3\n", pc );
		switch( cfg.mode ) {
		case JOINT_STEREO:
			pc = "joint stereo";
			break;
		...
		Jutil.lame_msgf( gfc, "\t%d channel - %s\n", cfg.channels_out, pc );
		...
		switch( cfg.vbr ) {
		case vbr_off:
			Jutil.lame_msgf(gfc, "\tconstant bitrate - CBR %s\n", pc );
			break;
		case vbr_abr:
			Jutil.lame_msgf(gfc, "\tvariable bitrate - ABR %s\n", pc );
			break;
		case vbr_rh:
			Jutil.lame_msgf(gfc, "\tvariable bitrate - VBR rh %s\n", pc );
			break;
		case vbr_mt:
			Jutil.lame_msgf(gfc, "\tvariable bitrate - VBR mt %s\n", pc );
			break;
		case vbr_mtrh:
			Jutil.lame_msgf(gfc, "\tvariable bitrate - VBR mtrh %s\n", pc );
			break;
		default:
			Jutil.lame_msgf(gfc, "\t ?? oops, some new one ?? \n" );
			break;
		}
		if( cfg.write_lame_tag ) {
			Jutil.lame_msgf( gfc, "\tusing LAME Tag\n" );
		}
		Jutil.lame_msgf( gfc, "\t...\n" );
		...
		Jutil.lame_msgf( gfc, "\tusing short blocks: %s\n", pc );
		Jutil.lame_msgf( gfc, "\tsubblock gain: %d\n", cfg.subblock_gain );
		Jutil.lame_msgf( gfc, "\tadjust masking: %g dB\n", gfc.sv_qnt.mask_adjust );
		Jutil.lame_msgf( gfc, "\tadjust masking short: %g dB\n", gfc.sv_qnt.mask_adjust_short );
		Jutil.lame_msgf( gfc, "\tquantization comparison: %d\n", cfg.quant_comp );
		Jutil.lame_msgf( gfc, "\t ^ comparison short blocks: %d\n", cfg.quant_comp_short );
		Jutil.lame_msgf( gfc, "\tnoise shaping: %d\n", cfg.noise_shaping );
		Jutil.lame_msgf( gfc, "\t ^ amplification: %d\n", cfg.noise_shaping_amp );
		Jutil.lame_msgf( gfc, "\t ^ stopping: %d\n", cfg.noise_shaping_stop );
		...
		Jutil.lame_msgf( gfc, "\tATH: %s\n", pc );
		Jutil.lame_msgf( gfc, "\t ^ type: %d\n", cfg.ATHtype );
		Jutil.lame_msgf( gfc, "\t ^ shape: %g%s\n", cfg.ATHcurve, " (only for type 4)" );
		Jutil.lame_msgf( gfc, "\t ^ level adjustement: %g dB\n", cfg.ATH_offset_db );
		Jutil.lame_msgf( gfc, "\t ^ adjust type: %d\n", gfc.ATH.use_adjust );
		Jutil.lame_msgf( gfc, "\t ^ adjust sensitivity power: %f\n", gfc.ATH.aa_sensitivity_p );

		Jutil.lame_msgf( gfc, "\texperimental psy tunings by Naoki Shibata\n" );
		Jutil.lame_msgf( gfc, "\t   adjust masking bass=%g dB, alto=%g dB, treble=%g dB, sfb21=%g dB\n",
			10 * Math.log10( gfc.sv_qnt.longfact[0]),
			10 * Math.log10( gfc.sv_qnt.longfact[7]),
			10 * Math.log10( gfc.sv_qnt.longfact[14]), 10 * Math.log10(gfc.sv_qnt.longfact[21]) );

		pc = cfg.use_temporal_masking_effect ? "yes" : "no";
		Jutil.lame_msgf( gfc, "\tusing temporal masking effect: %s\n", pc );
		Jutil.lame_msgf( gfc, "\tinterchannel masking ratio: %g\n", cfg.interChRatio );
		Jutil.lame_msgf( gfc, "\t...\n" );
	...
	private static final void save_gain_values(final Jlame_internal_flags gfc) {
		final JSessionConfig cfg = gfc.cfg;
		final JRpgStateVar rsv = gfc.sv_rpg;
		final JRpgResult rov = gfc.ov_rpg;
		/* save the ReplayGain value */
		if( cfg.findReplayGain ) {
			final float RadioGain = (float) rsv.rgdata.GetTitleGain();
			if( ! Jquantize.EQ( RadioGain, Jreplaygain.GAIN_NOT_ENOUGH_SAMPLES ) ) {
				rov.RadioGain = (int) Math.floor( RadioGain * 10.0f + 0.5f ); /* round to nearest */
			} else {
				rov.RadioGain = 0;
			}
		}
		...
		if( cfg.findPeakSample ) {
			rov.noclipGainChange = (int) Math.ceil( Math.log10( rov.PeakSample / 32767.0 ) * 20.0 * 10.0 ); /* round up */

			if( rov.noclipGainChange > 0 ) { /* clipping occurs */
				rov.noclipScale = (float)Math.floor((32767.0f / rov.PeakSample) * 100.0f) / 100.0f; /* round down */
			} else {
	...
	private static final int update_inbuffer_size(final Jlame_internal_flags gfc, final int nsamples) {
		...
		if( esv.in_buffer_0 == null || esv.in_buffer_1 == null ) {
			...
			Jutil.lame_errorf( gfc, "Error: can't allocate in_buffer buffer\n" );
			return -2;
		}
		return 0;
	}
	...
	private static final int calcNeeded(final JSessionConfig cfg) {;
		...
		int mf_needed = Jencoder.BLKSIZE + pcm_samples_per_frame - Jencoder.FFTOFFSET; /* amount needed for FFT */
	 ...
	 */
	private static final int lame_encode_buffer_sample_t(final Jlame_internal_flags gfc,
		int nsamples, final byte[] mp3buf, int mp3buf_offset, final int mp3buf_size)
	{
		...
		while( nsamples > 0 ) {
			...
			final long tmp = Jutil.fill_buffer( gfc, mfbuf, in_buffer, in_buffer_ptr, nsamples/*, &n_in, &n_out*/ );
			final int n_out = (int)tmp;
			final int n_in = (int)(tmp >> 32);

			/* compute ReplayGain of resampled input if requested */
			if( cfg.findReplayGain && ! cfg.decode_on_the_fly) {
				if( gfc.sv_rpg.rgdata.AnalyzeSamples( mfbuf[0], mfbuf[1], esv.mf_size, n_out, cfg.channels_out )
			...
			if( esv.mf_size >= mf_needed ) {
				...
				final int ret = Jencoder.lame_encode_mp3_frame( gfc, mfbuf[0], mfbuf[1], mp3buf, mp3buf_offset, buf_size );
	 ...
	 */
	private static final void lame_copy_inbuffer(final Jlame_internal_flags gfc,
	...
	private static final int lame_encode_buffer_template(final Jlame_global_flags gfp,
	 ...
	 *
	 */
	public static final int lame_encode_buffer(final Jlame_global_flags gfp,
	 ...
	 * short int, +/- 32768
	 */
	public static final int lame_encode_buffer_float(final Jlame_global_flags gfp,
	 ...
	 */
	public static final int lame_encode_buffer_ieee_float(final Jlame_global_flags gfp,
		final float pcm_l[], final float pcm_r[], final int nsamples,
		final byte[] mp3buf, final int mp3buf_offset, final int mp3buf_size)
	{
	...
	public static final int lame_encode_buffer_interleaved_ieee_float(final Jlame_global_flags gfp,
	 ...
	 * !! NOTE: !! data must be scaled to +/- 1 full scale
	 */
	public static final int lame_encode_buffer_ieee_double(final Jlame_global_flags gfp,
	...
	public static final int lame_encode_buffer_interleaved_ieee_double(final Jlame_global_flags gfp,
	...
	public static final int lame_encode_buffer_int(final Jlame_global_flags gfp,
	 ...
	 *
	 */
	public static final int lame_encode_buffer_long2(final Jlame_global_flags gfp,
	 ...
	 *
	 */
	public static final int lame_encode_buffer_long(final Jlame_global_flags gfp,
		final long pcm_l[], final long pcm_r[], final int nsamples,
		final byte[] mp3buf, final int mp3buf_offset, final int mp3buf_size)
	{
		/* input is assumed to be normalized to +/- 32768 for full scale */
	 ...
	 */
	public static final int lame_encode_buffer_interleaved(final Jlame_global_flags gfp,
	...
	public static final int lame_encode_buffer_interleaved_int(final Jlame_global_flags gfp,
	...
	/*****************************************************************
	 ...
	 with no errors.
	*********************************************************************/
	public static final int lame_encode_flush_nogap(final Jlame_global_flags gfp, final byte[] mp3buffer, int mp3buffer_size) {
		int rc = -3;
		if( gfp.is_lame_global_flags_valid() ) {
			final Jlame_internal_flags gfc = gfp.internal_flags;
			if( gfc.is_lame_internal_flags_valid() ) {
	...
	/*****************************************************************/
	public static final int lame_encode_flush(final Jlame_global_flags gfp, final byte[] mp3buffer, final int mp3buffer_size ) {
		if( ! gfp.is_lame_global_flags_valid() ) {
			return -3;
		}
		final Jlame_internal_flags gfc = gfp.internal_flags;
		...
		int samples_to_encode = esv.mf_samples_to_encode - Jencoder.POSTDELAY;

		final short buffer[][] = new short[2][1152];// java: already zeroed
		int mp3count = 0;

		final boolean is_resampling_necessary = Jutil.isResamplingNecessary( cfg );
	...
	*/
	public static final void lame_mp3_tags_fid(final Jlame_global_flags gfp, final RandomAccessFile fpStream ) {
		if( ! gfp.is_lame_global_flags_valid() ) {
			return;
		}
		final Jlame_internal_flags gfc = gfp.internal_flags;
		...
		try {
			if( fpStream != null ) {
				...
				default:
					...
					break;
				case -1:
					Jutil.lame_errorf( gfc, "Error: could not update LAME tag.\n" );
					break;
				case -2:
					Jutil.lame_errorf( gfc, "Error: could not update LAME tag, file not seekable.\n" );
					break;
				case -3:
					Jutil.lame_errorf( gfc, "Error: could not update LAME tag, file not readable.\n" );
					break;
				}
			}
	...
	private static final int lame_init_internal_flags(final Jlame_internal_flags gfc)
	{
		...
		gfc.cfg.decode_on_the_fly = false;
		gfc.cfg.findReplayGain = false;
		gfc.cfg.findPeakSample = false;
		...
		gfc.sv_enc.mf_size = Jencoder.ENCDELAY - Jencoder.MDCTDELAY; /* we pad input with this many 0's */
		...
		gfc.ov_rpg.noclipScale = -1.0f;

		gfc.ATH = new JATH();
		//if( null == gfc.ATH )
		//	return -2;      /* maybe error codes should be enumerated in lame.h ?? */

		gfc.sv_rpg.rgdata = new Jreplaygain();
	...
	private static final int lame_init_old(final Jlame_global_flags gfp) {
		...
		gfp.report.debugf = Jutil.lame_report_def;
		gfp.report.errorf = Jutil.lame_report_def;
		gfp.report.msgf = Jutil.lame_report_def;
	 ...
	 *
	 */
	public static final void lame_bitrate_kbps(final Jlame_global_flags gfp, final int bitrate_kbps[/* 14 */]) {
		if( gfp.is_lame_global_flags_valid() ) {
			final Jlame_internal_flags gfc = gfp.internal_flags;
			if( gfc.is_lame_internal_flags_valid() ) {
				final JSessionConfig cfg = gfc.cfg;
				if( cfg.free_format ) {
					...
					for( int i = 0; i < 14; i++) {
						bitrate_kbps[i] = Jtables.bitrate_table[cfg.version][i + 1];
					}
				}
			}
		}
	...
	public static final void lame_bitrate_hist(final Jlame_global_flags gfp, final int bitrate_count[/* 14 */]) {
		if( gfp.is_lame_global_flags_valid() ) {
			final Jlame_internal_flags gfc = gfp.internal_flags;
			if( gfc.is_lame_internal_flags_valid() ) {
				final JSessionConfig cfg = gfc.cfg;
	...
	public static final void lame_stereo_mode_hist(final Jlame_global_flags gfp, final int stmode_count[/* 4 */]) {
		if( gfp.is_lame_global_flags_valid() ) {
			final Jlame_internal_flags gfc = gfp.internal_flags;
			if( gfc.is_lame_internal_flags_valid() ) {
				final JEncResult eov = gfc.ov_enc;
	...
	public static final void lame_bitrate_stereo_mode_hist(final Jlame_global_flags gfp, final int bitrate_stmode_count[][]/* [14][4] */) {
		if( gfp.is_lame_global_flags_valid() ) {
			final Jlame_internal_flags gfc = gfp.internal_flags;
			if( gfc.is_lame_internal_flags_valid() ) {
				final JSessionConfig cfg = gfc.cfg;
	...
	public static final void lame_block_type_hist(final Jlame_global_flags gfp, final int btype_count[/* 6 */]) {
		if( gfp.is_lame_global_flags_valid() ) {
			final Jlame_internal_flags gfc = gfp.internal_flags;
			if( gfc.is_lame_internal_flags_valid() ) {
				final JEncResult eov = gfc.ov_enc;
	...
	public static final void lame_bitrate_block_type_hist(final Jlame_global_flags gfp, final int bitrate_btype_count[][]/* [14][6] */) {
		if( gfp.is_lame_global_flags_valid() ) {
			final Jlame_internal_flags gfc = gfp.internal_flags;
			if( gfc.is_lame_internal_flags_valid() ) {
				final JSessionConfig cfg = gfc.cfg;
