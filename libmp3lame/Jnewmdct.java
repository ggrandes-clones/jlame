...
************************************************************************/
	...
	private static final void mdct_short(final float[] inout, int offset) {
		for( int l = 0; l < 3; l++ ) {
			...
			tc0 = (inout[offset + 1 * 3] * win[Jencoder.SHORT_TYPE][1] - inout[offset + 4 * 3]) * 2.069978111953089e-11f; /* tritab_s[1] */
			ts0 = (inout[offset + 4 * 3] * win[Jencoder.SHORT_TYPE][1] + inout[offset + 1 * 3]) * 2.069978111953089e-11f; /* tritab_s[1] */
	...
	private static final void mdct_long(final float[] out, final int outoffset, final float[] in) {
		{
			...
			ct = tc1 * win[Jencoder.SHORT_TYPE][12+0] + tc2 + tc3 * win[Jencoder.SHORT_TYPE][12+1] + tc4 * win[Jencoder.SHORT_TYPE][12+2];
			st = -ts5 * win[Jencoder.SHORT_TYPE][12+4] + ts6 - ts7 * win[Jencoder.SHORT_TYPE][12+5] + ts8 * win[Jencoder.SHORT_TYPE][12+3];
			out[outoffset + 1] = ct + st;
			out[outoffset + 2] = ct - st;

			ct = tc1 * win[Jencoder.SHORT_TYPE][12+1] - tc2 - tc3 * win[Jencoder.SHORT_TYPE][12+2] + tc4 * win[Jencoder.SHORT_TYPE][12+0];
			st = -ts5 * win[Jencoder.SHORT_TYPE][12+5] + ts6 - ts7 * win[Jencoder.SHORT_TYPE][12+3] + ts8 * win[Jencoder.SHORT_TYPE][12+4];
			out[outoffset + 9] = ct + st;
			out[outoffset + 10] = ct - st;

			ct = tc1 * win[Jencoder.SHORT_TYPE][12+2] - tc2 + tc3 * win[Jencoder.SHORT_TYPE][12+0] - tc4 * win[Jencoder.SHORT_TYPE][12+1];
			st = ts5 * win[Jencoder.SHORT_TYPE][12+3] - ts6 + ts7 * win[Jencoder.SHORT_TYPE][12+4] - ts8 * win[Jencoder.SHORT_TYPE][12+5];
		...
		}
		{
			...
			ct = tc5 * win[Jencoder.SHORT_TYPE][12+3] - tc6 + tc7 * win[Jencoder.SHORT_TYPE][12+4] + tc8 * win[Jencoder.SHORT_TYPE][12+5];
			st = ts1 * win[Jencoder.SHORT_TYPE][12+2] + ts2 + ts3 * win[Jencoder.SHORT_TYPE][12+0] + ts4 * win[Jencoder.SHORT_TYPE][12+1];
			out[outoffset + 3] = ct + st;
			out[outoffset + 4] = ct - st;

			ct = -tc5 * win[Jencoder.SHORT_TYPE][12+5] + tc6 - tc7 * win[Jencoder.SHORT_TYPE][12+3] - tc8 * win[Jencoder.SHORT_TYPE][12+4];
			st = ts1 * win[Jencoder.SHORT_TYPE][12+1] + ts2 - ts3 * win[Jencoder.SHORT_TYPE][12+2] - ts4 * win[Jencoder.SHORT_TYPE][12+0];
			out[outoffset + 7] = ct + st;
			out[outoffset + 8] = ct - st;

			ct = -tc5 * win[Jencoder.SHORT_TYPE][12+4] + tc6 - tc7 * win[Jencoder.SHORT_TYPE][12+5] - tc8 * win[Jencoder.SHORT_TYPE][12+3];
			st = ts1 * win[Jencoder.SHORT_TYPE][12+0] - ts2 + ts3 * win[Jencoder.SHORT_TYPE][12+1] - ts4 * win[Jencoder.SHORT_TYPE][12+2];
