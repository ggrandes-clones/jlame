...
final class Jquantize_pvt {
	...
	*/
	static final Jscalefac_struct sfBandIndex[] = {// [9] = {
		new Jscalefac_struct(                   /* Table B.2.b: 22.05 kHz */
		...
		new Jscalefac_struct(                   /* Table B.2.c: 24 kHz */ /* docs: 332. mpg123(broken): 330 */
		...
		new Jscalefac_struct(                   /* Table B.2.a: 16 kHz */
			new int[]{0, 6, 12, 18, 24, 30, 36, 44, 54, 66, 80, 96, 116, 140, 168, 200, 238, 284, 336, 396, 464, 522, 576},
			new int[]{0, 4, 8, 12, 18, 26, 36, 48, 62, 80, 104, 134, 174, 192},
			new int[]{0, 0, 0, 0, 0, 0, 0}, /*  sfb21 pseudo sub bands */
			new int[]{0, 0, 0, 0, 0, 0, 0} /*  sfb12 pseudo sub bands */
		),
		new Jscalefac_struct(                   /* Table B.8.b: 44.1 kHz */
		...
		new Jscalefac_struct(                   /* Table B.8.c: 48 kHz */
		...
		new Jscalefac_struct(                   /* Table B.8.a: 32 kHz */
		...
		new Jscalefac_struct(                   /* MPEG-2.5 11.025 kHz */
			new int[]{0, 6, 12, 18, 24, 30, 36, 44, 54, 66, 80, 96, 116, 140, 168, 200, 238, 284, 336, 396, 464, 522, 576},
			new int[]{0 / 3, 12 / 3, 24 / 3, 36 / 3, 54 / 3, 78 / 3, 108 / 3, 144 / 3, 186 / 3, 240 / 3, 312 / 3, 402 / 3, 522 / 3, 576 / 3},
			new int[]{0, 0, 0, 0, 0, 0, 0}, /*  sfb21 pseudo sub bands */
			new int[]{0, 0, 0, 0, 0, 0, 0} /*  sfb12 pseudo sub bands */
		),
		new Jscalefac_struct(                   /* MPEG-2.5 12 kHz */
			new int[]{0, 6, 12, 18, 24, 30, 36, 44, 54, 66, 80, 96, 116, 140, 168, 200, 238, 284, 336, 396, 464, 522, 576},
			new int[]{0 / 3, 12 / 3, 24 / 3, 36 / 3, 54 / 3, 78 / 3, 108 / 3, 144 / 3, 186 / 3, 240 / 3, 312 / 3, 402 / 3, 522 / 3, 576 / 3},
			new int[]{0, 0, 0, 0, 0, 0, 0}, /*  sfb21 pseudo sub bands */
			new int[]{0, 0, 0, 0, 0, 0, 0} /*  sfb12 pseudo sub bands */
		),
		new Jscalefac_struct(                   /* MPEG-2.5 8 kHz */
...
//#endif
	...
	*/
	...
	private static final void compute_ath(final Jlame_internal_flags gfc) {
		...
		final float samp_freq = cfg.samplerate_out;
...
//}
	 ...
	 ************************************************************************/
	static final int on_pe(final Jlame_internal_flags gfc, final float pe[][/*2*/], final int targ_bits[/*2*/], final int mean_bits, final int gr, final boolean cbr)
	{
		final JSessionConfig cfg = gfc.cfg;
		int extra_bits = 0;
	...
	static final float athAdjust(final float a, final float x, final float athFloor, final float ATHfixpoint) {
		...
		if( v > 1E-20f ) {
			w = 1.f + ((float)Math.log10( (double)v ) * (10.0f / o));// Jutil.FAST_LOG10_X( v, 10.0f / o );
		}
		if( w < 0 ) {
			w = 0.f;
		}
	...
	*/
	static final int calc_xmin(final Jlame_internal_flags gfc,
		final JIII_psy_ratio ratio, final Jgr_info cod_info, final float[] pxmin)
	{
		...
		if( cod_info.block_type != Jencoder.SHORT_TYPE ) { /* NORM, START or STOP type, but not SHORT */
		...
		if( ! gfc.sv_qnt.sfb21_extra && cfg.samplerate_out < 44000 ) {
			final int sfb_l = (cfg.samplerate_out <= 8000) ? 17 : 21;
			final int sfb_s = (cfg.samplerate_out <= 8000) ?  9 : 12;
			int limit;// = 575;
			if( cod_info.block_type != Jencoder.SHORT_TYPE ) { /* NORM, START or STOP type, but not SHORT */
	...
	private static final float calc_noise_core_c(final Jgr_info cod_info, final int[] startline, int l, final float step) {
		float noise = 0;
		int j = startline[0];
	...
	/* + 6 dB  =>  +3.69 */
	/* +10 dB  =>  +6.45 */
	static final int calc_noise(final Jgr_info cod_info,
		final float[] l3_xmin,
		final float[] distort, final Jcalc_noise_result res, final Jcalc_noise_data prev_noise)
	{
		...
		for( int sfb = 0; sfb < cod_info.psymax; sfb++ ) {
			...
			if( prev_noise != null && (prev_noise.step[sfb] == s) ) {
				...
				noise = (float)Math.log10( (double)(distort_ >= 1E-20f ? distort_ : 1E-20f) );// Jutil.FAST_LOG10( (distort_ >= 1E-20f ? distort_ : 1E-20f) );
	 ...
	 *
	 ************************************************************************/
	private static final void set_pinfo(final Jlame_internal_flags gfc,
		final Jgr_info cod_info, final JIII_psy_ratio ratio, final int gr, final int ch)
	{
		final JSessionConfig cfg = gfc.cfg;
		final float ifqstep = (cod_info.scalefac_scale == 0) ? .5f : 1.0f;
		final int[] scalefac = cod_info.scalefac;

		final float l3_xmin[] = new float[Jencoder.SFBMAX];
		final float xfsf[] = new float[Jencoder.SFBMAX];
		final Jcalc_noise_result noise = new Jcalc_noise_result();
		...
		if( cod_info.block_type != Jencoder.SHORT_TYPE && !cod_info.mixed_block_flag ) {
			sfb2 = 22;
		}
		final float[] ratio_en_l = ratio.en.l;// java
		final double[] en_gr_ch = gfc.pinfo.en[gr][ch];// java
		final double[] xfsf_gr_ch = gfc.pinfo.xfsf[gr][ch];// java
		final double[] thr_gr_ch = gfc.pinfo.thr[gr][ch];// java
		final double[] LAMEsfb_gr_ch = gfc.pinfo.LAMEsfb[gr][ch];// java

		int sfb = 0;
		for( ; sfb < sfb2; sfb++ ) {
			final int start = gfc.scalefac_band.l[sfb];
			final int end = gfc.scalefac_band.l[sfb + 1];
			final int bw = end - start;
			float en0 = 0.0f;
			for( ; j < end; j++ ) {
			...
			final float en1 = 1e15f;     /* scaling so it shows up on FFT plot */
			en_gr_ch[sfb] = en1 * en0;
			xfsf_gr_ch[sfb] = en1 * l3_xmin[sfb] * xfsf[sfb] / bw;

			if( ratio_en_l[sfb] > 0 && ! cfg.ATHonly ) {
				en0 = en0 / ratio_en_l[sfb];
			} else {
				en0 = 0.0f;
			}

			thr_gr_ch[sfb] = en1 * Math.max( en0 * ratio.thm.l[sfb], gfc.ATH.l[sfb] );

			/* there is no scalefactor bands >= SBPSY_l */
			LAMEsfb_gr_ch[sfb] = 0;
			if( cod_info.preflag && sfb >= 11 ) {
				LAMEsfb_gr_ch[sfb] = -ifqstep * pretab[sfb];
			}

			if( sfb < Jencoder.SBPSY_l ) {
				/* scfsi should be decoded by caller side */
				LAMEsfb_gr_ch[sfb] -= ifqstep * scalefac[sfb];
			}
		}                   /* for sfb */
		...
		if( cod_info.block_type == Jencoder.SHORT_TYPE ) {
			...
			final float[] xr = cod_info.xr;// java
			final double[] en_s = gfc.pinfo.en_s[gr][ch];// java
			final double[] xfsf_s = gfc.pinfo.xfsf_s[gr][ch];// java
			final double[] thr_s = gfc.pinfo.thr_s[gr][ch];// java
			final double[] LAMEsfb_s = gfc.pinfo.LAMEsfb_s[gr][ch];// java
			sfb2 = sfb;
			for( sfb = cod_info.sfb_smin; sfb < Jencoder.SBMAX_s; sfb++ ) {
				final int start = s[sfb];
				final int end = s[sfb + 1];
				final int bw = end - start;
				for( int i = 0; i < 3; i++ ) {
					float en0 = 0.0f;
					...
					en0 = Math.max( en0 / bw, 1e-20f );
					/* convert to MDCT units */
					final float en1 = 1e15f; /* scaling so it shows up on FFT plot */

					en_s[3 * sfb + i] = en1 * en0;
					xfsf_s[3 * sfb + i] = en1 * l3_xmin[sfb2] * xfsf[sfb2] / bw;
					...
					thr_s[3 * sfb + i] = en1 * Math.max( en0 * ratio.thm.s[sfb][i], gfc.ATH.s[sfb] );

					/* there is no scalefactor bands >= SBPSY_s */
					LAMEsfb_s[3 * sfb + i] = -2.0 * cod_info.subblock_gain[i];
					if( sfb < Jencoder.SBPSY_s ) {
						LAMEsfb_s[3 * sfb + i] -= ifqstep * scalefac[sfb2];
					}
					sfb2++;
				}
			}
	 ...
	 *
	 ************************************************************************/
	static final void set_frame_pinfo(final Jlame_internal_flags gfc, final JIII_psy_ratio ratio[][]/*[2][2]*/) {
		final JSessionConfig cfg = gfc.cfg;
		final Jgr_info[][] tt = gfc.l3_side.tt;// java
		final Jgr_info[] tt0 = tt[0];// java

		/* for every granule and channel patch l3_enc and set info */
		for( int gr = 0, mode_gr = cfg.mode_gr, channels_out = cfg.channels_out; gr < mode_gr; gr++ ) {
			final JIII_psy_ratio[] ratio_gr = ratio[gr];// java
			final Jgr_info[] tt_gr = tt[gr];// java
			for( int ch = 0; ch < channels_out; ch++ ) {
				final Jgr_info cod_info = tt_gr[ch];
				final int scalefac_sav[] = new int[Jencoder.SFBMAX];
				final int[] scalefac = cod_info.scalefac;// java
				System.arraycopy( scalefac, 0, scalefac_sav, 0, Jencoder.SFBMAX );

				/* reconstruct the scalefactors in case SCFSI was used */
				if( gr == 1 ) {
					for( int sfb = 0, sfb_lmax = cod_info.sfb_lmax; sfb < sfb_lmax; sfb++ ) {
						if( scalefac[sfb] < 0 ) {
							scalefac[sfb] = tt0[ch].scalefac[sfb];
						}
					}
				}

				set_pinfo( gfc, cod_info, ratio_gr[ch], gr, ch );
				System.arraycopy( scalefac_sav, 0, scalefac, 0, Jencoder.SFBMAX );
			}               /* for ch */
		}                   /* for gr */
	}
}