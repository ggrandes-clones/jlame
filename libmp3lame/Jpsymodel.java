...
/* #ifndef NDEBUG
	...
	private static final float vbrpsy_mask_add(float m1, float m2, final int b, final int delta) {
		...
		if( ( b >= 0 ? b : -b ) <= delta ) {       /* approximately, 1 bark = 3 partitions */
			...
			if( ratio >= ma_max_i1 ) {
				...
				final int i = (int) ((float)Math.log10( (double)ratio ) * (16.0f));// (Jutil.FAST_LOG10_X( ratio, 16.0f ));
    ...
    between them
	 */
	private static final void convert_partition2scalefac(final JPsyConst_CB2SB gd, final float[] eb, final float[] thr,
	...
	private static final void convert_partition2scalefac_s(final Jlame_internal_flags gfc, final float[] eb, final float[] thr, final int chn,
		final int sblock)
	{
	...
	private static final void convert_partition2scalefac_l(final Jlame_internal_flags gfc, final float[] eb, final float[] thr, final int chn)
	{
		final JPsyStateVar psv = gfc.sv_psy;
	...
	private static final void convert_partition2scalefac_l_to_s(final Jlame_internal_flags gfc, final float[] eb, final float[] thr, final int chn)
	{
		final JPsyStateVar psv = gfc.sv_psy;
				...
				if( thm > 0.0f ) {
					...
					if( en > x ) {
						if( en > x * 1e10f ) {
							...
							pe_s += regcoef_s[sb] * (float)Math.log10( (double)(en / x) );// Jutil.FAST_LOG10( en / x );
						}
					}
				}
			}
			...
			if( thm > 0.0f ) {
				...
				if( en > x ) {
					if( en > x * 1e10f ) {
						...
						pe_l += regcoef_l[sb] * (float)Math.log10( (double)(en / x) );// Jutil.FAST_LOG10( en / x );
					}
				}
			}
		}
	...
	private static final void calc_energy(final JPsyConst_CB2SB l, final float[] fftenergy, final float[] eb, final float[] max, final float[] avg)
	{
	...
	private static final void calc_mask_index_l(final Jlame_internal_flags gfc, final float[] max, final float[] avg, final byte[] mask_idx)
	{
		final JPsyConst_CB2SB gdl = gfc.cd_psy.l;
	...
	private static final void vbrpsy_compute_fft_l(final Jlame_internal_flags gfc,
		...
		final float wsamp_l[/*BLKSIZE*/][], final int woffset)
	{
		...
		final JPsyStateVar psv = gfc.sv_psy;
		final Jplotting_data plt = cfg.analysis ? gfc.pinfo : null;
		...
		*********************************************************************/
		...
		if( plt != null ) {
			for( int j = 0; j < Jencoder.HBLKSIZE; j++ ) {
	...
	private static final void vbrpsy_compute_fft_s(final Jlame_internal_flags gfc,
		final float buffer[/*2*/][], final int boffset,// java
		final int chn,
	...
	*********************************************************************/
	private static final void vbrpsy_compute_loudness_approximation_l(final Jlame_internal_flags gfc, final int gr_out, final int chn,
	...
	**********************************************************************/
	private static final void vbrpsy_attack_detection(final Jlame_internal_flags gfc,
		final float buffer[/*2*/][], int boffset,
		final int gr_out,
		final JIII_psy_ratio masking_ratio[/*2*/][/*2*/], final JIII_psy_ratio masking_MS_ratio[/*2*/][/*2*/],
	...
	private static final void vbrpsy_skip_masking_s(final Jlame_internal_flags gfc, final int chn, final int sblock) {
		if( sblock == 0 ) {
	...
	private static final void vbrpsy_calc_mask_index_s(final Jlame_internal_flags gfc, final float[] max,
	...
	private static final void vbrpsy_compute_masking_s(final Jlame_internal_flags gfc, final float fftenergy_s[/*HBLKSIZE_s*/][],
...
// #endif
	...
	private static final void vbrpsy_compute_masking_l(final Jlame_internal_flags gfc, final float fftenergy[/*HBLKSIZE*/],
...
// #endif
	...
	private static final void vbrpsy_compute_block_type(final JSessionConfig cfg, final boolean[] uselongblock) {
		...
		final boolean is_short_block_dispensed = (short_blocks == Jlame_global_flags.short_block_dispensed);
	...
	private static final void vbrpsy_apply_block_type(final JPsyStateVar psv, final int nch, final boolean[] uselongblock, final int[] blocktype_d) {
	 ...
	 */
	static final int L3psycho_anal_vbr(final Jlame_internal_flags gfc,
		...
		final float energy[/*4*/], final int blocktype_d[/*2*/])
	{
		...
		*********************************************************************/
		final boolean blocktype = (blocktype_d[0] == Jencoder.SHORT_TYPE || blocktype_d[1] == Jencoder.SHORT_TYPE);// java
		for( int chn = 0; chn < n_chn_psy; chn++ ) {
			float[] appe;// java
			int ppe, type;
			JIII_psy_ratio mr;
...
/* #if 0
	...
	private static final void init_numline(final JPsyConst_CB2SB gd, float sfreq, final int fft_size,
	...
	private static final void compute_bark_values(final JPsyConst_CB2SB gd, float sfreq, final int fft_size,
	...
	static final int psymodel_init(final Jlame_global_flags gfp) {
		...
		final float sfreq = cfg.samplerate_out;
		...
		psv.blocktype_old[0] = psv.blocktype_old[1] = Jencoder.NORM_TYPE; /* the vbr header is long blocks */
		...
		for( int i = 0; i < gd.l.npart; i++ ) {
			...
			for( int k = 0; k < gd.l.numlines[i]; k++, j++ ) {
				...
				level = Jutil.ATHformula( cfg, freq * 1000 ) - 20; /* scale to FFT units; returned value is in dB */
		...
		if( cfg.ATHtype != -1 ) {
			...
			final float freq_inc = (float) cfg.samplerate_out / (float) (Jencoder.BLKSIZE);
			...
			for( int i = Jencoder.BLKSIZE / 2; --i >= 0; ) { /* scale weights */
