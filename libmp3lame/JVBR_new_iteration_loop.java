...
final class JVBR_new_iteration_loop extends Jquantize/* implements Jiteration_loop */{
	 ...
	 */
	private static final int VBR_new_prepare(final Jlame_internal_flags gfc,
		final float pe[][]/*[2][2]*/, final JIII_psy_ratio ratio[][]/*[2][2]*/,
	...
	// @Override
	static final void iteration(final Jlame_internal_flags gfc, final float pe[][], final float ms_ratio[], final JIII_psy_ratio ratio[][] ) {
		final JSessionConfig cfg = gfc.cfg;
		final JEncResult eov = gfc.ov_enc;
...
/* #if 0
		...
		if( used_bits <= frameBits[eov.bitrate_index] ) {
			...
			Jutil.lame_errorf( gfc, "INTERNAL ERROR IN VBR NEW CODE, please send bug report\n" );
			System.exit( -1 );
			return;
		//}
	}
