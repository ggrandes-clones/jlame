...
//}
	...
	private static final boolean tri_calc_sfb_noise_x34(final float[] xr, final int xroffset, final float[] xr34, final int xr34offset, final float l3_xmin, final int bw,
		final int sf, final Jcalc_noise_cache[] did_it)
	{
...
/* a variation for vbr-mtrh */
	private static final int block_sf(final Jalgo_s that, final float l3_xmin[/*SFBMAX*/], final int vbrsf[/*SFBMAX*/], final int vbrsfmin[/*SFBMAX*/])
	{
...
*/
	...
	private static final void set_subblock_gain(final Jgr_info cod_info, final int mingain_s[/*3*/], final int sf[]) {
		...
		for( int i = 0; i < 3; ++i ) {
			...
			for( ; sfb < Jencoder.SFBMAX; sfb += 3 ) { /* part 2 */
				final int v = -sf[sfb];
				if( maxsf2 < v ) {
					maxsf2 = v;
				}
...
*/
	private static final void set_scalefacs(final Jgr_info cod_info, final int[] vbrsfmin, final int sf[], final byte[] max_range) {
		final int ifqstep = (cod_info.scalefac_scale == 0) ? 2 : 4;
		final int ifqstepShift = (cod_info.scalefac_scale == 0) ? 1 : 2;
...
// #ifndef NDEBUG
	private static final boolean checkScalefactor(final Jgr_info cod_info, final int vbrsfmin[/*SFBMAX*/]) {
		final int ifqstep = cod_info.scalefac_scale == 0 ? 2 : 4;
		for( int sfb = 0; sfb < cod_info.psymax; ++sfb ) {
			final int s = ((cod_info.scalefac[sfb] +
					(cod_info.preflag ? Jquantize_pvt.pretab[sfb] : 0)) * ifqstep) +
 ...
 *
 ******************************************************************/
	private static final void short_block_constrain(final Jalgo_s that, final int vbrsf[/*SFBMAX*/],
	...
	*
	******************************************************************/
	private static final void long_block_constrain(final Jalgo_s that, final int vbrsf[/*SFBMAX*/], final int vbrsfmin[/*SFBMAX*/],
		int vbrmax)
	{
		final Jgr_info cod_info = that.cod_info;
		final Jlame_internal_flags gfc = that.gfc;
	...
	private static final void bitcount(final Jalgo_s that) {
		...
		Jutil.lame_errorf( that.gfc, "INTERNAL ERROR IN VBR NEW CODE (986), please send bug report\n" );
		System.exit( -1 );
	}
	...
	private static final int tryGlobalStepsize(final Jalgo_s that, final int sfwork[/*SFBMAX*/],
	...
	private static final void searchGlobalStepsizeMax(final Jalgo_s that, final int sfwork[/*SFBMAX*/],
	...
	private static final int tryThatOne(final Jalgo_s that, final int sftemp[/*SFBMAX*/], final int vbrsfmin[/*SFBMAX*/], final int vbrmax) {
		final float xrpow_max = that.cod_info.xrpow_max;
		int nbits = Jtakehiro.LARGE_BITS;
		// that.alloc( that, sftemp, vbrsfmin, vbrmax );
		if( that.alloc == short_block_constrain ) {
	...
	private static final void outOfBitsStrategy(final Jalgo_s that, final int sfwork[/*SFBMAX*/], final int vbrsfmin[/*SFBMAX*/], final int target)
	{
...
/* #if 0
	...
	static final int VBR_encode_frame(final Jlame_internal_flags gfc, final float xr34orig[][][]/*[2][2][576]*/,
		final float l3_xmin[][][]/*[2][2][SFBMAX]*/, final int max_bits[][]/*[2][2]*/)
	{
		...
		int max_nbits_fr = 0;
		final int use_nbits_ch[][] = {{Jutil.MAX_BITS_PER_CHANNEL+1, Jutil.MAX_BITS_PER_CHANNEL+1},{Jutil.MAX_BITS_PER_CHANNEL+1, Jutil.MAX_BITS_PER_CHANNEL+1}};
		...
		Jutil.lame_errorf( gfc, "INTERNAL ERROR IN VBR NEW CODE (1313), please send bug report\n" +
