...
import libmp3lame.JVBRTAGDATA;
import libmp3lame.JVbrTag;
import libmp3lame.Jplotting_data;
...
public class Jmpstr_tag {
	private static final class Jbuf {
		...
		private int pos;
		private Jbuf next;
		//private Jbuf prev;// FIXME prev never uses
	}
	//
	...
	/* #if 0
		...
		// private static final int HDRCMPMASK = 0xfffffd00;

	/* #if 0
		private static final void print_header(final Jmpstr_tag mp, final Jframe fr) {
			final String modes[] = { "Stereo", "Joint-Stereo", "Dual-Channel", "Single-Channel" };
			final String layers[] = { "Unknown", "I", "II", "III" };

			Jutil.lame_report_fnc( mp.report_msg, "MPEG %s, Layer: %s, Freq: %ld, mode: %s, modext: %d, BPF : %d\n",
					fr.mpeg25 ? "2.5" : (fr.lsf != 0 ? "2.0" : "1.0"),
							layers[fr.lay], freqs[fr.sampling_frequency],
							modes[fr.mode], fr.mode_ext, fr.framesize + 4 );
			Jutil.lame_report_fnc( mp.report_msg, "Channels: %d, copyright: %s, original: %s, CRC: %s, emphasis: %d.\n",
					fr.stereo, fr.copyright ? "Yes" : "No",
							fr.original ? "Yes" : "No", fr.error_protection ? "Yes" : "No", fr.emphasis );
			Jutil.lame_report_fnc( mp.report_msg, "Bitrate: %d Kbits/s, Extension value: %d\n",
					tabsel_123[fr.lsf][fr.lay - 1][fr.bitrate_index], fr.extension );
		}

		private static final void print_header_compact(final Jmpstr_tag mp, final Jframe fr) {
			final String modes[] = { "stereo", "joint-stereo", "dual-channel", "mono" };
			final String layers[] = { "Unknown", "I", "II", "III" };

			Jutil.lame_report_fnc( mp.report_err, "MPEG %s layer %s, %d kbit/s, %ld Hz %s\n",
					fr.mpeg25 ? "2.5" : (fr.lsf != 0 ? "2.0" : "1.0"),
							layers[fr.lay],
	...
	public final boolean InitMP3() {
		...
		this.report_dbg = Jutil.lame_report_def;
		this.report_err = Jutil.lame_report_def;
		this.report_msg = Jutil.lame_report_def;
		// }
	...
	private final Jbuf addbuf(final byte[] buf, final int size) {
		...
		/*if( !nbuf.pnt ) {
			free(nbuf);// FIXME nbuf must be free
			return NULL;
		}*/
		nbuf.size = size;
	...
	private final int read_buf_byte() {
		...
		while( pos >= this.tail.size ) {
			remove_buf();
			if( null == this.tail ) {
				Jutil.lame_report_fnc(this.report_err, "hip: Fatal error! tried to read past mp buffer\n");
				System.exit( 1 );
				return 0;
			}
			pos = this.tail.pos;
		...
		final Isynth synth)
	{
		...
		if( ! this.header_parsed ) {
			...
			if( bytes > 0 ) {
				 ...
				 */
				if( this.fsizeold != -1 ) {
					Jutil.lame_report_fnc( this.report_err, "hip: bitstream problem, resyncing skipping %d bytes...\n", bytes );
				}
				this.old_free_format = false;
// #if 1
				/* FIXME: correct ??? */
...
// #endif
				...
				if( size > Jmpg123.MAXFRAMESIZE ) {
					...
					Jutil.lame_report_fnc( this.report_err, "hip: wordpointer trashed.  size=%d (%d)  bytes=%d \n",
			...
			if( this.fr.lay == 3 ) {
				...
				if( ! this.free_format ) {
					...
					if( this.dsize > frame_size ) {
						Jutil.lame_report_fnc( this.report_err,
		...
		if( ! this.data_parsed ) {
			...
			case 3:
				...
				break;
			default:
				Jutil.lame_report_fnc( this.report_err, "hip: invalid layer %d\n", this.fr.lay );
			}
...
// #endif
			...
			if( size > Jmpg123.MAXFRAMESIZE ) {
				Jutil.lame_report_fnc( this.report_err, "hip: fatal error.  MAXFRAMESIZE not large enough.\n");
			}
		}
	...
	@SuppressWarnings("boxing")
	public final int decodeMP3(final byte[] in, final int isize, final Object out, final int osize, final int[] done) {
		if( osize < 2304/* * (Short.SIZE / 8)*/ ) {// FIXME must be 2304 * sizeof( short )
			Jutil.lame_report_fnc( this.report_err, "hip: Insufficient memory for decoding buffer %d\n", osize );
			return Jmpg123.MP3_ERR;
		}
	...
	public final int decodeMP3_unclipped(final byte[] in, final int isize, final Object out, final int osize, final int[] done) {
		// we forbid input with more than 1152 samples per channel for output in unclipped mode
		if( osize < (int) (1152 * 2/* * (Float.SIZE / 8)*/) ) {
			Jutil.lame_report_fnc( this.report_err, "hip: out space too small for unclipped mode\n");
			return Jmpg123.MP3_ERR;
		}
	...
	final int set_pointer(final int backstep) {
		if( this.fsizeold < 0 && backstep > 0 ) {
			Jutil.lame_report_fnc( this.report_err, "hip: Can't step back %d bytes!\n", backstep );
			return Jmpg123.MP3_ERR;
		}
